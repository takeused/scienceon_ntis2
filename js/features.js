...
// ──────────────────────────────────────────────────────────────
// Feature 4: 논문 키워드 연관 네트워크 시각화
// (실제 인용 데이터 없음 → 키워드 공유 기반 논문 유사도 네트워크)
// ──────────────────────────────────────────────────────────────

function showPaperNetwork() {
  if (!STATE.currentItems || STATE.currentItems.length === 0) {
    showToast('먼저 검색을 실행하세요.', 'warning'); return;
  }
  const modal = document.getElementById('paperNetworkModal');
  if (!modal) return;
  document.getElementById('paperNetworkTitle').textContent = `"${STATE.currentQuery}" 논문 연관 네트워크`;
  modal.classList.remove('hidden');
  renderPaperNetwork(STATE.currentItems, STATE.currentQuery);
}

function closePaperNetwork() {
  document.getElementById('paperNetworkModal')?.classList.add('hidden');
}

function renderPaperNetwork(items, centerQuery) {
  const container = document.getElementById('paperNetworkContainer');
  if (!container || typeof d3 === 'undefined') {
    if (container) container.innerHTML = '<p class="text-center text-gray-400 py-16">D3 라이브러리 로딩 중...</p>';
    return;
  }
  container.innerHTML = '';
  ...
  // 논문별 키워드 추출 (최대 20개 항목)
  const paperItems = items.filter(it => it.keywords || it.title).slice(0, 20);
  if (paperItems.length === 0) {
    container.innerHTML = '<p class="text-center text-gray-400 py-16">키워드 데이터가 없어 네트워크를 생성할 수 없습니다.</p>';
    return;
  }

  // 논문 노드 생성
  const nodes = paperItems.map((it, i) => ({
    id: i,
    label: (it.title || '').substring(0, 30) + ((it.title || '').length > 30 ? '…' : ''),
    fullTitle: it.title || '',
    keywords: (it.keywords || '').split(/[;,|]/).map(k => k.trim()).filter(Boolean),
    year: it.year || '',
    group: 1,
  }));
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (shared.length >= 1) {
        links.push({ source: i, target: j, weight: shared.length, shared });
      }
    }
  }
  ...
  nodes.forEach(n => { n.size = connectedIds.has(n.id) ? 10 : 6; });
  ...
  const sim = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(links).id(d => d.id).distance(d => Math.max(60, 120 - d.weight * 15)))
    .force('charge', d3.forceManyBody().strength(-180))
    .force('center', d3.forceCenter(W / 2, H / 2))
    .force('collision', d3.forceCollide(d => d.size + 12));

  // 링크 색: 공유 키워드 수에 따라 굵기 & 색 변화
  const linkEl = svg.append('g').selectAll('line').data(links).join('line')
    .attr('stroke', d => d3.interpolateBlues(Math.min(d.weight / 4, 1)))
    .attr('stroke-width', d => Math.min(1 + d.weight, 4))
    .attr('opacity', 0.6)
    .append('title').text(d => `공유 키워드: ${d.shared.join(', ')}`);

  const linkEl2 = svg.selectAll('line');
  ...
  nodeEl.append('circle')
    .attr('r', d => d.size)
    .attr('fill', d => connectedIds.has(d.id) ? '#111' : '#ccc')
    .attr('stroke', '#fff')
  ...
  nodeEl.append('title').text(d => `${d.fullTitle}\n${d.year ? '(' + d.year + ')' : ''}\n키워드: ${d.keywords.slice(0, 5).join(', ')}`);

  sim.on('tick', () => {
    linkEl2
      .attr('x1', d => Math.max(d.source.size, Math.min(W - d.source.size, d.source.x)))
      .attr('y1', d => Math.max(d.source.size, Math.min(H - d.source.size, d.source.y)))
      .attr('x2', d => Math.max(d.target.size, Math.min(W - d.target.size, d.target.x)))
      .attr('y2', d => Math.max(d.target.size, Math.min(H - d.target.size, d.target.y)));
  ...
  legend.append('rect').attr('width', 200).attr('height', 28).attr('fill', 'white').attr('opacity', 0.85).attr('rx', 4);
  legend.append('circle').attr('cx', 14).attr('cy', 14).attr('r', 6).attr('fill', '#111');
  legend.append('text').attr('x', 26).attr('y', 18).attr('font-size', '10px').attr('fill', '#555').text('연관 논문 (키워드 공유)');
  legend.append('circle').attr('cx', 145).attr('cy', 14).attr('r', 5).attr('fill', '#ccc');
  legend.append('text').attr('x', 156).attr('y', 18).attr('font-size', '10px').attr('fill', '#999').text('키워드 없음');
}
