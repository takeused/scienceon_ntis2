...
Respond ONLY with this JSON structure:
{
  ...
  <div class="p-6 rounded-2xl" style="background: var(--surface); border: 1px solid rgba(239,68,68,0.2);">
    ...
    function showKeywordNetwork() {
      if (!STATE.currentItems || STATE.currentItems.length === 0) {
        showToast('먼저 검색을 실행하세요.', 'warning'); return;
      }
      document.getElementById('networkTitle').textContent = `"${STATE.currentQuery}" 키워드 네트워크`;
      document.getElementById('networkModal').classList.add('open');
      renderNetwork(STATE.currentItems, STATE.currentQuery);
    }
    ...
    function renderNetwork(items, centerQuery) {
      ...
      const topKws = Object.entries(kwFreq).sort((a,b) => b[1]-a[1]).slice(0, 30);
      ...
      const link = svg.append('g').selectAll('line').data(links).join('line')
        .attr('class', 'link').attr('stroke', '#ddd').attr('stroke-width', 1.2);
      ...
      node.append('circle')
        .attr('r', d => d.size)
        .attr('fill', d => d.group === 0 ? '#111' : '#f5f5f5')
        .attr('stroke', d => d.group === 0 ? '#111' : '#ddd');

      node.append('text')
        .attr('dy', d => d.size + 12)
        .attr('text-anchor', 'middle')
        .attr('font-size', d => d.group === 0 ? '13px' : '11px')
        .attr('font-weight', d => d.group === 0 ? 'bold' : 'normal')
        .attr('fill', d => d.group === 0 ? '#111' : '#666')
        .text(d => d.id.length > 12 ? d.id.slice(0,12)+'…' : d.id);

      sim.on('tick', () => {
        link.attr('x1', d => d.source.x).attr('y1', d => d.source.y)
            .attr('x2', d => d.target.x).attr('y2', d => d.target.y);
        node.attr('transform', d => `translate(${Math.max(d.size, Math.min(W-d.size, d.x))},${Math.max(d.size, Math.min(H-d.size, d.y))})`);
      });
    }
